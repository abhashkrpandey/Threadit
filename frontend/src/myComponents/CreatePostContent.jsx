...
export default function CreatePostContent() {
    ...
    async function postfunc() {
        if (title.length < 4) {
            ...
            const response = await axios.post(import.meta.env.VITE_BACKEND_URL + "/createpost",
                {
                    posttitle: title,
                    postbody: postbody,
                    userid: userid,
    ...
    async function searchCommunityFunc() {
        const response = await axios.post(import.meta.env.VITE_BACKEND_URL + "/searchcommunity", {
            ...
            <div className=" flex flex-col gap-2">
                ...
                <div>
                    <div className="flex flex-row ">
                        <input id="community_name" placeholder="select a  community" className="bg-gray-300 rounded-sm w-[20vw] h-[5vh] focus:outline-none " onChange={postcontent} required></input>
                        {arraySubRedditName.length === 0 ? (
                            ...
                            <ul>
                                {arraySubRedditName.map((ele) => {
                                    return (
                                        <li key={ele._id.toString()} onClick={() => (communityNameSetter(ele._id.toString(), ele.subname))} className="hover:bg-gray-300">{ele.subname}</li>
                                    )
                                })}
                            </ul>
                        )}
                        <button className="bg-amber-600" onClick={searchCommunityFunc}>search</button>
                    </div>
                </div>
                <div>
                    <input id="title" maxLength={300} type="text" placeholder="Title" className="bg-gray-300 rounded-sm w-[50vw] h-[5vh]" onChange={postcontent}></input>
                    <div>{(title.length)}/300</div>
                </div>
                <div>
                    <textarea id="body" placeholder="Body Text(optional)" className="bg-gray-300 rounded-sm  w-[50vw]" onChange={postcontent}></textarea>
                </div>
                <div className="flex flex-row justify-end gap-2">
                    <button className="bg-blue-600">SaveDraft</button>
                    <button className="bg-blue-600" onClick={postfunc}>Post</button>
                </div>
            </div>
        </>
    )
